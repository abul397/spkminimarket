...
function tambahDataAlternatif($nama, $kriteria){
	...
	$query  = "SELECT id FROM alternatif ORDER BY id DESC LIMIT 1";
	$result = mysqli_query($koneksi, $query);
	...
	for ($i=0; $i < count($k); $i++) {
		$query 	= "INSERT INTO detail_alternatif (id_alternatif, id_kriteria, nilai) VALUES ('$alt', '$k[$i]', '$kriteria[$i]')";
		$tambah	= mysqli_query($koneksi, $query);
	}
...
function deleteAlternatif($id) {
	...
	$query 	= "DELETE FROM detail_alternatif WHERE id_alternatif=$id";
	mysqli_query($koneksi, $query);
